# labeling/image_cluster_labeler.py
"""
Two-tab Streamlit tool for IMAGE (DINO patch) cluster labeling:
...
Run with: streamlit run labeling/image_cluster_labeler.py
"""
import sys, os
...
def run_streamlit():
    ...
    with tab_label:
        ...
        n_samples = st.sidebar.slider('Samples per cluster', 10, 75, 25, 5)
        ...
        # Sidebar: Clickable cluster list — grouped by label, noise at bottom
        st.sidebar.header('Clusters')
        cls_emoji = {"cat": "🐱", "car": "🚗", "bird": "🐦"}
        _noise_labels = {'noise', 'background', 'bg', '—', ''}

        def _render_cluster_btn(cid):
            ...
            is_noise = clabel.strip().lower() in _noise_labels
            emoji = '❓' if is_noise else cls_emoji.get(dom, '⬜')
        ...
        # Group clusters: labeled (non-noise) sorted by label name, then unlabeled/noise
        labeled_cids = sorted(
            [cid for cid in range(n_clusters)
             if existing.get(str(cid), {}).get('label', '').strip().lower() not in _noise_labels
             and existing.get(str(cid), {}).get('label', '')],
            key=lambda cid: existing[str(cid)]['label'].lower()
        )
        noise_cids = [cid for cid in range(n_clusters) if cid not in labeled_cids]

        if labeled_cids:
            st.sidebar.caption('— labeled —')
            for cid in labeled_cids:
                _render_cluster_btn(cid)
        if noise_cids:
            st.sidebar.caption('— unlabeled / noise —')
            for cid in noise_cids:
                _render_cluster_btn(cid)
