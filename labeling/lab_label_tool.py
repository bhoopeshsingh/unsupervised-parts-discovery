...
Run with:  streamlit run labeling/lab_label_tool.py
"""
...
def _show_classification_results(score_vec, concept_names, cfg, cache_dir):
    ...
    clf = ConceptClassifier.load(clf_path)
    score_vec_scaled = clf.scaler.transform(score_vec)
    pred_class_id    = clf.clf.predict(score_vec_scaled)[0]
    pred_proba_all   = clf.clf.predict_proba(score_vec_scaled)[0]
    pred             = clf.class_names[pred_class_id]
    ...
    with st.expander("Class probability breakdown"):
        rows = sorted(
            [{"Condition": clf.class_names[i].replace("_"," ").title(),
...
def run_streamlit():
    ...
    tab_label, tab_classify = st.tabs(["🏷️ Label Clusters", "🔍 Classify Record"])
